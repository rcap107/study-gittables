...
def parse_args():
    ...
    subparsers = parser.add_subparsers(help="Select the command to execute.", required=True)
    
    parser_extract = subparsers.add_parser("extract", help="Extract archives to their subfolders.")
    parser_extract.add_argument("-s", "--src_dir", action="store", required=True, help="Path to data folder "
                                "that contains the archives. ")
    parser_extract.add_argument("-d", "--dest_dir", action="store", required=True, help="Path to data folder "
                                "that will contain the extracted folders.")
    parser_extract.add_argument("-v", "--verbose", action="store_true", help="Print archive size for all archives.")
    parser_extract.set_defaults(func=extract_tables)
    
    parser_convert = subparsers.add_parser("convert", help="Convert tables to pseudo-text format.")
    parser_convert.add_argument("-p", "--data_folder", action="store", required=True, help="Path to the folder "
                                "where tables are stored. ASSUMES PARQUET FORMAT.")
    parser_convert.add_argument("--n_jobs", action="store", type=int, default=1, help="Number of jobs used by joblib.")
